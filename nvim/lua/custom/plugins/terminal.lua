...
local runners = {
	...
	typescript = {
		run = function(path, dir)
			return npm_script('start', dir) or
			    (exists(dir .. '/tsconfig.json') and 'ts-node ' .. vim.fn.shellescape(path)) or
			    'tsx ' .. vim.fn.shellescape(path)
		end,
	},
	lua = {
		run = function(path)
	...
	c = {
		run = function(path, dir)
			...
			return make_cmd(nil, dir)
			    ...
			    or
			    (string.format('cc %s -O2 -Wall -o %s && %s', vim.fn.shellescape(path), vim.fn.shellescape(out), vim.fn.shellescape(out)))
		end,
	},
	cpp = {
		run = function(path, dir)
			...
			return make_cmd(nil, dir)
			    or cmake_build_cmd(dir)
			    or
			    (string.format('c++ %s -std=c++20 -O2 -Wall -o %s && %s', vim.fn.shellescape(path), vim.fn.shellescape(out), vim.fn.shellescape(out)))
		end,
	},
	rust = {
		run = function(path, dir)
			...
			return string.format('rustc %s -o %s && %s', vim.fn.shellescape(path),
				vim.fn.shellescape(out_file), vim.fn.shellescape(out_file))
		end,
	},
}
...
local function run_in_term(cmd, dir)
	...
	runner_terminal = Terminal:new {
		...
		on_open = function(term)
			...
			vim.api.nvim_buf_set_keymap(term.bufnr, 't', '<Esc><Esc>', [[<C-\><C-n>:hide<CR>]],
				{ noremap = true, silent = true })
...
return {
	...
	config = function()
		...
		map({ 'n', 't' }, '<A-r>', function()
			...
			local cmd = (runners[ft] and runners[ft].run) and runners[ft].run(path, dir) or
			    make_cmd(nil, dir) or cargo_cmd('run', dir)
